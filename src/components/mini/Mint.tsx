...
import { useContractWrite, useBalance, useNetwork, useAccount } from 'wagmi';
...
function Mint() {
  ...
  const notEnoughETH = (balance?.value ?? 0) < mintPrice;
  ...
  const mintToken = async () => {
    try {
      ...
      if ((error as Error)?.message.includes('missing revert data')) {
  ...
  return (
    <div className="lg:pl-60 mint bg-grdeen-900 w-full lg:w-2/4 flex flex-col items-center  ">
      ...
      <h3 className="text-xl lg:text-2xl self-center justify-self-center align mt-5 text-center">3333 Baldies</h3>
      ...
      <div className="mintsection flex flex-col justify-center items-center mb-10">
        {chain?.id === 8453 ? (
          ...
          >
            {notEnoughETH ? 'Not enough ETH' : <img src={mintnow} alt="" className="w-44 mb-5 lg:w-64 lg:mb-2" />}
          </button>
        ) : (
          <div className="mb-8">
            <ConnectButton />
